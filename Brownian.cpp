...
const double dt = 0.001; //Intervalo temporal (Para todos iguales menos para los del Depor que para ellos seguimos estando en 2001).
...
int Brow_steps = 10000;    // Número de saltos con movimiento Browniano (Tardan lo mismo en darse todos que lo que tarda Koke en bajar a defender).
int nsamp_pos = 500;    // Ratio con el que tomamos las medidas del Browniano.
...
int tsteps = Brow_steps*0.1 / nsamp_pos;  //Esto es para la normalización de la RDF. Es el tiempo que hay entre medida y medida. 
...
int main() {
    ...
    int naccept = 0;//Mide el número de saltos aceptados.
    double EBnew, EBold; //Variables para estudiar la energía después y antes de los saltos respectivamente.

    //Bucle Browniano.
    for (int bstep = 0; bstep < Brow_steps; ++bstep) {     
        for (int i = 0; i < N_part * N_dim; ++i) Pos_Old[i] = Pos_New[i];
        vector<double>Force = Force_over_particle(N_part, Pos_Old); //Generamos todas las fuerzas que actúan sobre las distintas partículas en el instante de tiempo t.

        //Conocidas las fuerzas generamos las posiciones en el tiempo t+dt
        for (int j = 0; j < N_part; j++) {
            for (int k = 0; k < N_dim; k++) {
                Pos_New[N_dim * j + k] = Pos_Old[N_dim * j + k] + mu * Force[N_dim * j + k] * dt + sqrt(2 * D * dt) * rand_normal(0.0, 1.0);
            }


        }
        //Aceptación o rechazo de los saltos. Esta es la forma más fácil de evitar explosiones en el programa y prohibir configuraciones poco favorables.
        EBold = Energy_Brownian(Pos_Old);
        EBnew = Energy_Brownian(Pos_New);

        //Esto si se pilla el dt bien no hay porque ponerlo. El dt de ahora de 0.001 va bastante bien. Para dt mayores esto hace de filtro para evitar saltos a configuraciones poco favorables y explosiones.

        //Exactamente igual que en el Monte Carletto.
        double prob = exp(-(EBnew - EBold) / temp);
        double xi = uniform(0, 1);

        if (prob < xi) {
            for (int k = 0; k < N_dim * N_part; k++) Pos_New[k] = Pos_Old[k]; //Si no se acepta el salto se vuelve a la configuración del tiempo t.
        }
        else naccept += 1; //Número de saltos aceptados
    ...
    cout << "Número de saltos aceptados: "<<naccept << endl; 
